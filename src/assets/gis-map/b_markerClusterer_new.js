...
(function () {
  ...
  var MarkerClusterer =
     ...
     */
    BMapLib.MarkerClusterer = function (map, options, cb) {
      if (!map) {
        return;
      }
      this._map = map;
      ...
      var opts = options || {};
      // 首页地图的自定带的聚合点默认隐藏级别
      this._hiddenZoom = opts["hiddenZoom"] || 0;
      this._gridSize = opts["gridSize"] || 60;
      this._maxZoom = opts["maxZoom"] || 18;
      this._minClusterSize = opts["minClusterSize"] || 1;
      this._isAverageCenter = false;
      // 原始展开，默认没有值，需要使用原始聚合点逻辑打开来提升性能
      this._isOriginalLaunch = opts["isOriginalLaunch"];
      ...
      //去掉绑定的地图放大和移动事件  因为在点击标记定位时调用 setViewport  后会多次触发地图移动事件和缩放事件
   ...
   * @return 无返回值。
   */
  MarkerClusterer.prototype.addMarker = function (marker, isRender) {
    this._pushMarkerTo(marker);
    // 报错 Cannot read property 'dl' of null
    // if (isRender) this._createClusters();
    this._createClusters();
  };
  ...
  MarkerClusterer.prototype.redraw = MarkerClusterer.prototype._redraw;
  ...
  /**
   * 获取聚合的隐藏级别。
   * @return {Number} 聚合的隐藏级别。
   */
  MarkerClusterer.prototype.getHiddenZoom = function () {
    return this._hiddenZoom;
  };
/**
   * 获取原始展开值。
   * @return {Number} 原始展开的值。
   */
  MarkerClusterer.prototype.getIsOriginalLaunch = function () {
    return this._isOriginalLaunch;
  };
  ...
  Cluster.prototype.addMarker = function (marker) {
    ...
    // 是否为原始展开，聚合点js更新之后不是原始展开（点击展开有弹性且有动画）
    if(!this._markerClusterer.getIsOriginalLaunch()){
      var len = this._markers.length;
  ...
  Cluster.prototype.render = function () {
    var len = this._markers.length;
    // 如果地图有设置隐藏层级，并且地图缩放层级大于首页隐藏层级
    if (len < this._minClusterSize || (this._markerClusterer.getHiddenZoom() && this._map.getZoom() >= this._markerClusterer.getHiddenZoom())) {
      for (var i = 0; i < len; i++) {
  ...
  Cluster.prototype.updateClusterMarker = function () {
    ...
    // 如果地图有设置隐藏层级，并且地图缩放层级大于首页隐藏层级，地图自带聚合点隐藏
    if (this._map.getZoom() >= this._markerClusterer.getHiddenZoom() && this._markerClusterer.getHiddenZoom()) {
      this._clusterMarker.hide();
      return;
    }
    ...
    // 计算总共设施总数
    this._markers.map(item => {
      if (item.allData) {
        if (item.allData.count) {
          countMount += item.allData.count;
        } else {
          item.allData.equipmentData.forEach(item => {
            countMount += item.count
          })
        }
      }
    });
    if (countMount) {
      this._clusterMarker.setText(countMount);
    } else {
      this._clusterMarker.setText(this._markers.length);
    }
    ...
    var thatMap = this._map;
    const thatMarkerClusterer = this._markerClusterer;
    var thatBounds = this.getBounds();

    // this._clusterMarker.addEventListener("click", function (event) {
    //   thatMap.setViewport(thatBounds);
    // });
    // var __maxZoom = this._markerClusterer._maxZoom;
    this._clusterMarker.removeEventListener("click");
    ...
    // } else {
      this._clusterMarker.addEventListener("click", function (event) {
        /**
         * 定制化地图（首页）每次点击clusterMarker聚合点放大至区域点13层级
         */
        if(thatMarkerClusterer.getHiddenZoom()) {
          thatMap.centerAndZoom(event.point, 13);
        } else {
          /**
           * setViewport后会多次触发地图移动事件和缩放事件
           * 导致多次请求后台资源
           */
          thatMap.setViewport(thatBounds);
        }
